...
#include "Characters/Enemy/FPMonsterStatComponent.h"
#include "FPGameInstance.h"

// Sets default values for this component's properties
UFPMonsterStatComponent::UFPMonsterStatComponent()
{
...
void UFPMonsterStatComponent::BeginPlay()
{
	Super::BeginPlay();
...
void UFPMonsterStatComponent::InitializeComponent()
{
...
void UFPMonsterStatComponent::SetHeal(float NewRecoverHP)
{
...
void UFPMonsterStatComponent::SetDamage(float NewDamage)
{
	FPCHECK(nullptr != CurrentStatData);
	SetHP(FMath::Clamp<float>(CurrentHP - NewDamage, 0.0f, CurrentStatData->MaxHP));
}

void UFPMonsterStatComponent::SetHP(float NewHP)
{
	if (CurrentHP < NewHP)
		IsRecoveringHP = true;
...
float UFPMonsterStatComponent::GetAttack()
{
	FPCHECK(nullptr != CurrentStatData, 0.0f);
	return CurrentStatData->Attack;
}

float UFPMonsterStatComponent::GetHPRatio()
{
	FPCHECK(nullptr != CurrentStatData, 0.0f);
	return (CurrentStatData->MaxHP < KINDA_SMALL_NUMBER) ? 0.0f : (CurrentHP / CurrentStatData->MaxHP);
}

int32 UFPMonsterStatComponent::GetDropExp() const
{
